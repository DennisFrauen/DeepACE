...
def perform_experiments(run_config, load_hyper=True, method_params=None, return_results=False):
    ...
    T = run_config["T"]
    n = run_config["n"]
    vital_list = run_config["vitals"]
    static_list = run_config["static_features"]
    ...
    for i in range(number_exp):
        ...
        #Estimate ACEs
        # Data
        d_train_seq, d_train_seq_unscaled, ates, a_int_1, a_int_2, y_scaler = generate_data(run_config, data_vitals, plot=False)
...
def generate_data(config, data_vitals, plot = False):
    T = config["T"]
    # Intervention sequences
    a_int_1, a_int_2 = sim.generate_interventions(T, config["treat"], config["dataset"])

    d_seq_unscaled, d_seq_cf_unscaled1, d_seq_cf_unscaled2 = mimic.simulate_treat_outcomes(config, a_int_1, a_int_2, data_vitals)
...
def estimate_ace(data_scaled, data_unscaled, a_int_1, a_int_2, method, hyperparams, run_config, y_scaler = None):
    ...
    if method in ["dml_forest", "dml_kernel", "static", "dr_forest"]:
...
if __name__ == "__main__":
    ...
    stream = open(helpers.get_project_path() + "/experiments/config/mimic/config_deepace_tar.yaml", 'r')
